...
(define (extend-env/module module-def env)
  (cases ModuleDef module-def
    ...
    (ModDefInterface (ivar interf) env)
  )
)
...
(define (value-of-module-body module-body export-names env)
  (define (recurse mb) (value-of-module-body mb export-names env))
  (cases ModuleBody module-body
    ...
    (MBProc (params its body)
      (module-proc-val (Procedure params body env))
    )
    (MBCall (mproc margs)
      (define mp (expval->module-proc (recurse mproc)))
      (apply-module-procedure mp (map recurse margs) export-names)
    )
  )
)

(define (uncons-names names var)
  (if names (values (car names) (cdr names)) (values var false))
)
(define (contruct-binding defs names eval-env)
  (match defs
    ((quote ()) (empty-env))
    ((cons def defs/)
      (cases MDefinition def
        (MDefVar (ident expr)
          ...
          (define eval-env/ (extend-env ident val eval-env))
          (define-values (name names/) (uncons-names names ident))
          (if (equal? name ident)
            (extend-env ident val (contruct-binding defs/ names/ eval-env/))
            (contruct-binding defs/ names eval-env/)
          )
        )
        (else (contruct-binding defs/ (and names (cdr names)) eval-env))
      )
    )
  )
)
